...
> 重要：扩展不读取暗牌与“真实身份”来决定“谁是谁”（仅用公开事件与引擎评分规则）；但为了做一些保守门槛，少量地方会读取“真实身份字段”做布尔判断（例如“忠臣是否已全部阵亡”），详见对应机制文档。
...
- 可整体开关（默认开启）
- 可在“脚本插件管理”里逐个启用/禁用、调整加载顺序
- 每个脚本可通过 HookBus 注入规则、接管个别武将 AI、做团队配合等

本仓库自带多个脚本（热门选将影响、铁索队友配合、裴秀/武诸葛/界沮授 AI 接管或加强等）。
