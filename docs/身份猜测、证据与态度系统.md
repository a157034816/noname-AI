...
- `docs/敌友判断因素1.md` 末尾已包含“当前实现口径（扩展）”，本文在其基础上补齐代码级细节与阈值/公式。
...
3. **不读取暗牌/不直接用真实身份做推断**：
   - “推断谁像忠/反/内”主要使用公开行为 + 引擎评分口径（`get.result(...)`、事件链等）。
   - 少量地方会读取 `player.identity` 做**门禁级布尔判断**（例如“忠臣是否已全死”），目的在于更保守地避免误伤，且不对外展示真实身份。
