...
- 想做“强门槛”（直接禁止某选择）：用更高优先级，并在必要时 `ctx.stop=true`
...
4) 达成确认后执行：铁索连环目标选择 + 属性伤害传导（并包含大量门槛避免误伤）
...
- 订阅 `slqj_ai_score`：对 chooseCard/chooseTarget 等评分注入影响
