...
### 5.2 若有“已暴露敌人”，优先处理已暴露敌人，避免继续盲打未暴露目标
（`_shownEnemyFirstHookInstalled`）

在身份局：

- 仅对“有害行为”（tv<0）生效
- 若候选集中存在“已暴露敌人”的可用目标（同样 tv<0）

则对未暴露目标施加大惩罚（强行把分数压下去）。

其中“已暴露敌人”定义为：

- 身份明置（`identityShown===true`）且对其态度为负
- 或：软暴露（`shown>=0.7`）且身份猜测明确（`guess!=unknown` 且 `confidence>=0.55`），并猜测为敌方阵营

效果：只要桌上已经有人“暴露为敌”，AI 更少把输出浪费在未暴露目标身上。
...
在出牌阶段的【杀】决策中，允许极端情况下“先斩盟友开路，再斩下家敌人”：

- 场景：你的下家是盟友（att>0.6），其下家是敌人（att<-0.6），且当前因距离原因打不到该敌人
- 条件：具备多次出杀能力（诸葛连弩/无限杀/额外杀），且手里【杀】数量足以覆盖“击杀盟友 + 击杀敌人”
