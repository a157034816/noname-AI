...
- **scripts 插件**：默认开启自动加载 `scripts/`；也可以用“scripts 插件管理”单独启用/禁用并调整加载顺序（通常需重启生效）。

---

## 内置 scripts 快速介绍（玩家）

> 说明：scripts 属于“可选插件层”。你可以在扩展设置里的“scripts 插件管理”逐个禁用、调整顺序；保存后通常需要重启生效。

- `01_popular_general_candidates.js`（热门武将候选偏置）：影响开局 AI 选将候选列表，让热门/强势武将更容易被选中
...
- `22_xin_jushou_zhuge_bonus.js`（界沮授武将红利：摸牌偏置诸葛连弩）：干预摸牌顺序让诸葛连弩更容易被摸到；会影响随机性/公平性，不需要该效果请关闭
