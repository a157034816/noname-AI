...
用途示例：
- scripts 插件想“固定某种人格/偏置某个 traits”可以在此时修改 `payload.persona.traits`（只影响本局）。
...
> 追踪口径用于“模型推进/记录”，由 `isAiPersonaTrackedPlayer(...)` 决定：当前实现中自机始终被追踪（用于面板展示与托管接管的数据连续性）；决策门禁仍由 `isLocalAIPlayer(...)` 控制（自机仅在托管时视为本地 AI）。
