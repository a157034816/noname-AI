...
| `slqj_ai_update_auto_check` | 启动时自动检查更新 | `true` | 启动时按当前更新通道自动检查 GitHub Releases | `src/update/auto_check.js` | 只检查不自动下载 |
| `slqj_ai_update_channel` | 更新通道 | `stable` | 选择跟踪 stable 正式版或 PR 测试版 | `src/update/settings.js`、`src/update/auto_check.js`、`src/update/update_modal.js` | stable 与 PR 之间切换时会强制重新下载安装 |
| （无键） | 选择 PR 测试版 | - | 打开可选 PR 列表，只显示开放且已有测试包的 PR；点选后自动保存目标 PR 并立即检查更新 | `src/update/pr_selector_modal.js`、`src/update/pr_selector.js` | 不自动下载；真正安装仍在“检查更新/更新”里确认 |
| `slqj_ai_update_pr_number` | （隐藏/由选择器维护） | 空 | 保存当前选中的目标 PR 编号（仅 PR 通道生效） | `src/update/settings.js`、`src/update/auto_check.js`、`src/update/update_modal.js` | 切到不同 PR 也会强制重新下载安装；由列表点选写入 |
...
- `stable`：只看正式版 release（tag 形如 `vX.Y.Z`）
- `pr`：只看指定 PR 的测试版 prerelease（版本形如 `0.0.1-pr.<pr>.<run>`）
- 当前安装包会记录自己的构建通道；因此当你在 stable / PR 之间切换，或从 PR#A 切到 PR#B 时，更新器会直接判定为“需要重新下载安装目标包”，不会拿两个通道的版本号硬比大小
