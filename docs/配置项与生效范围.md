...
| `slqj_ai_scripts_registry` | （隐藏/由管理器维护） | 空 | scripts 插件启用/顺序注册表 | `src/scripts_registry.js` `src/scripts_loader.js` | 由“scripts 插件管理”UI 保存 |
| （无键） | scripts 插件管理 | - | 打开管理 UI（启用/禁用/排序/保存） | `src/scripts_manager_modal.js` | 保存后通常需重启生效 |
