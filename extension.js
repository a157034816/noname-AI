...
export default function(){
		...
		precontent:function(config){
			const isDebug = () => {
				...
				try {
					if (config?.slqj_ai_scripts_debug) return true;
				} catch (e) {}
				try {
					if (lib?.config?.slqj_ai_scripts_debug) return true;
				} catch (e) {}
				return false;
			};
    ...
    "身临其境的AI": [
		...
		'<div style="margin:10px"><b>内置 scripts（默认随扩展安装，可在“脚本插件管理”禁用/排序）</b></div>',
		'<ul style="margin-top:0">',
		'<li><b>热门武将候选影响</b>：影响开局 AI 选将候选列表，让热门/强势武将更容易被选中。</li>',
		...
		'<li><b>界沮授武将红利（摸牌影响诸葛连弩）</b>：干预摸牌顺序让诸葛连弩更容易被摸到；会影响随机性/公平性，不需要该效果请关闭。</li>',
    ...
    intro: "为本地单机 AI 增加人格/心智模型、独立身份猜测、AI 面板与 脚本插件系统, 修改出牌/少数技能策略。\n 因为个人精力有限, 无法大量进行测试, 所以我很需要听到你们的声音...",
